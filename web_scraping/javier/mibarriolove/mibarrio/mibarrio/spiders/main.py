...
class MainSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.mibarrio.love']
    ...
    def second_parse(self, response, **kwargs):
        ...
        logo_name = download_images.download(logo, title_for_image=title_for_logo, nombre_del_lugar=self.name)

        links = response.xpath('//div[@class="seller-items"]/ul/li//div[@class="mf-product-thumbnail"]/a/@href').getall()
        for idx, link in enumerate(links):
            if link:
                logger.info(f'Products {main_link}/{idx}/{len(links)}')
                yield response.follow(link, callback=self.last_parse, cb_kwargs={'logo':logo_name})
    ...
    def last_parse(self, response, **kwargs):
        ...
        image_name = download_images.download(image, title_for_image=title_for_image, nombre_del_lugar=self.name)
