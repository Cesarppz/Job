...
class Webscrape(scrapy.Spider):
    ...
    custom_settings= {
                        'FEED_URI':f'results_test_{name}_{dia}_{mes}.csv',
                        'FEED_FORMAT':'csv',
                        'FEED_EXPORT_ENCODING':'utf-8', 
                        'PLAYWRIGHT_DEFAULT_NAVIGATION_TIMEOUT': '3000000',
            ...
            yield scrapy.Request(
                ...
                meta = {
                    ...
                    "playwrite_page_methods":[
                        ...
                        PageMethod("evaluate", "window.scrollBy(0,document.body.scrollHeight)"),
                        PageMethod("wait_for_selector", "#CybotCookiebotDialogBodyLevelButtonLevelOptinAllowAll"),

                        # PageMethod("wait_for_selector", "#divbalances"),
                        # PageMethod("evaluate", "window.scrollBy(0,document.body.scrollHeight)"),

                        PageMethod("wait_for_selector", 'td:contains("EBITDA")'),
                        PageMethod("evaluate", "window.scrollBy(0,document.body.scrollHeight)")
                    ], 
    ...
    async def parse(self, response):
        ...
        cuentas_anueales_years =  await extract_item( response.xpath('//tr[contains(., "Ingresos")]/td[@class="noborder "]//text()').get())
        time.sleep(5)
        await page.close()
        # income =  
